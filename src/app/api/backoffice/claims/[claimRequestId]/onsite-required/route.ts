...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (message.startsWith("MISSING_")) return NextResponse.json({ ok: false, error: "Faltan catálogos base para visita presencial." }, { status: 500 });
    const status = getStatus(error);
    return NextResponse.json({ ok: false, error: status === 403 ? "No tienes permisos para revisar reclamos." : status === 401 ? "No autenticado." : "No se pudo marcar visita presencial." }, { status });
  }
}
