...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    return NextResponse.json({ ok: false, error: status === 403 ? "No tienes permisos para revisar claims." : status === 401 ? "No autenticado." : "No se pudo aprobar la verificación presencial." }, { status });
  }
}
