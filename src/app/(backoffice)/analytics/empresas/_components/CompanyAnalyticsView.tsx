...
import { CompanyScoreHistoryChart } from "./CompanyScoreHistoryChart";
import { CompanyScoreTable } from "./CompanyScoreTable";

type CompanyScoreItem = {
  companyId: number;
  companyName: string;
  finalScore: number;
    ...
    async function load() {
      try {
        ...
        const response = await fetch(
          `/api/backoffice/analytics/companies?${queryString}`,
          {
            method: "GET",
            credentials: "include",
            cache: "no-store",
        ...
        const payload = (await response.json()) as {
          ok: boolean;
          data?: CompanyAnalyticsResponse;
          error?: string;
        };

        if (!response.ok || !payload.ok || !payload.data) {
          throw new Error(payload.error ?? "No se pudieron obtener los analytics de empresas.");
        }
        ...
        if (!cancelled) {
          ...
          setError(
            err instanceof Error
              ? err.message
              : "Ocurrió un error al cargar el analytics de empresas."
          );
        }
      } finally {
        if (!cancelled) {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-2 text-sm text-neutral-500">
          Score actual, tendencia histórica y ranking operativo de empresas.
        </p>
      </div>

      <SectionCard
        title="Filtros"
        description="Filtra por empresa o rango de fechas sobre los snapshots históricos."
      >
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <MetricCard
          label="Empresas con score"
          value={data?.summary.totalCompanies ?? 0}
          helper="Registros actuales en analytics_company_scores"
          loading={isLoading}
        />
        <MetricCard
      ...
      <CompanyScoreHistoryChart data={data?.history ?? []} />

      <CompanyScoreTable rows={data?.items ?? []} loading={isLoading} />
      ...
      {!isLoading && !error && data && data.items.length === 0 ? (
        <EmptyState
          title="No hay empresas para los filtros aplicados"
          description="Prueba con otro rango de fechas o quita el filtro por empresa."
        />
      ) : null}
    </div>
  );
