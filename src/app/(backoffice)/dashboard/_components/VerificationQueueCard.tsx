...
export function VerificationQueueCard({ data }: Props) {
  return <QueueCard title="Verificaciones" description="Validación de identidad empresarial" icon="shield" tone="indigo" data={data} finalLabel="Aprobadas" finalValue={data.approved ?? 0} />;
}

function QueueCard({ title, description, icon, tone, data, finalLabel, finalValue }: { title: string; description: string; icon: "shield"; tone: "indigo"; data: QueueMetric; finalLabel: string; finalValue: number }) {
    ...
    <article className="overflow-hidden rounded-[22px] border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.045)] dark:border-white/[0.075] dark:bg-[#101620]">
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-[14px] bg-indigo-50 text-indigo-600 dark:bg-indigo-500/10 dark:text-indigo-300"><AppIcon name={icon} className="h-[18px] w-[18px]" /></span><div><h3 className="text-sm font-bold text-slate-900 dark:text-white">{title}</h3><p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">{description}</p></div></div>
          <p className="text-2xl font-extrabold tracking-[-0.05em] text-slate-950 dark:text-white">{formatNumber(data.total)}</p>
        </div>
        <div className="mt-5 grid grid-cols-3 gap-2"><MiniMetric label="Pendientes" value={data.pending} /><MiniMetric label="En revisión" value={data.inReview} /><MiniMetric label={finalLabel} value={finalValue} /></div>
