  ...
  return (
    <article className="overflow-hidden rounded-[22px] border border-slate-200/80 bg-white shadow-[0_8px_30px_rgba(15,23,42,0.045)] dark:border-white/[0.075] dark:bg-[#101620]">
      ...
      <div className="p-5">
        ...
        <div className="mt-5 grid grid-cols-3 gap-2"><MiniMetric label="Pendientes" value={data.pending} /><MiniMetric label="En revisión" value={data.inReview} /><MiniMetric label="Descartadas" value={data.rejected ?? 0} /></div>
...
function Unavailable({ reason }: { reason?: string }) { return <article className="rounded-[22px] border border-dashed border-slate-300 bg-white p-5 dark:border-white/[0.12] dark:bg-[#101620]"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-[14px] bg-slate-100 text-slate-500 dark:bg-white/[0.06]"><AppIcon name="reviews" className="h-[18px] w-[18px]" /></span><div><h3 className="text-sm font-bold text-slate-900 dark:text-white">Moderación</h3><p className="mt-1 text-[10px] text-slate-500 dark:text-slate-400">Reportes y revisión de reseñas</p></div></div><p className="mt-5 text-xs font-semibold text-amber-600 dark:text-amber-300">Datos no disponibles</p><p className="mt-1 line-clamp-2 text-[10px] text-slate-500 dark:text-slate-400">{reason ?? "No se pudo consultar el servicio."}</p></article>; }
