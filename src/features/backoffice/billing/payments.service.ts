...
function mapPaymentItem(raw: unknown): PaymentListItem {
  ...
  return {
    ...
    paymentMethodId: Number(
      item.paymentMethodId ?? item.payment_method_id ?? 0,
    ),
    paymentMethodName: String(
      item.paymentMethodName ?? item.payment_method_name ?? "Sin método",
    ),
    statusId:
    ...
    createdAt: String(
      item.createdAt ?? item.created_at ?? new Date(0).toISOString(),
    ),
  };
}
...
function mapPaymentSummary(
  ...
  const summary =
    typeof payload.summary === "object" && payload.summary !== null
      ...
      : payload;
  const money =
    typeof summary.money === "object" && summary.money !== null
      ? (summary.money as Record<string, unknown>)
      : {};
  return {
    totalAmount: Number(
      summary.totalAmount ??
    ...
    pendingPayments: Number(
      summary.pendingPayments ?? summary.pending_payments ?? 0,
    ),
...
export async function getPaymentsList(input: BillingListFilters = {}) {
  ...
  const raw = await callBilling<unknown>("/payments", {
