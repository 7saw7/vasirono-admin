...
function mapPlanItem(raw: unknown): PlanListItem {
  ...
  return {
    ...
    subscriptionsCount: Number(item.subscriptionsCount ?? item.subscriptions_count ?? 0),
    activeSubscriptionsCount: Number(item.activeSubscriptionsCount ?? item.active_subscriptions_count ?? 0),
    companiesCount: Number(item.companiesCount ?? item.companies_count ?? 0),
  };
}

function mapPlanSummary(raw: unknown, items: PlanListItem[], total: number): PlanSummary {
  ...
  const summary =
    typeof payload.summary === "object" && payload.summary !== null
      ...
      : payload;
  return {
    totalPlans: Number(summary.totalPlans ?? summary.total_plans ?? summary.total ?? total ?? 0),
...
export async function getPlansList(input: BillingListFilters = {}) {
  ...
  const raw = await callBilling<unknown>("/plans", { query: filtersQuery(filters) });
