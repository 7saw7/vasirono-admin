...
function normalizeReport(raw: unknown) {
  ...
  return {
    ...
    statusName: String(
      item.statusName ?? item.status_name ?? item.status ?? "Sin estado",
    ),
    statusCode: String(
      item.statusCode ?? item.status_code ?? item.status ?? "unknown",
    ).toLowerCase(),
...
function normalizeDetail(raw: unknown) {
  ...
  return {
    ...
    reviewRating: toNumber(
      payload.reviewRating ?? payload.review_rating ?? payload.rating,
    ),
    reviewCreatedAt: toIsoString(
      payload.reviewCreatedAt ??
        payload.review_created_at ??
        payload.createdAt ??
        payload.created_at,
    ),
  };
...
export async function getReviewReportDetail(reportId: number) {
  const raw = await callBackofficeService<unknown>(
    "reviewReports",
    `/api/backoffice/review-reports/${reportId}`,
  );
  ...
  const raw = await callBackofficeService<unknown>(
    ...
    `/api/backoffice/review-reports/${reportId}/resolve`,
    {
      ...
      body: {
        ...
        hideReason:
          parsed.status === "hidden" ? (parsed.resolutionNotes ?? null) : null,
      },
    },
  );
